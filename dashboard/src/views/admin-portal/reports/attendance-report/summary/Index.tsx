...
const Index = () => {
  ...
  const [states, setStates] = useState<StatsData>({
    total_students: 0,
    total_presents: 0,
    total_absents: 0,
    total_lates: 0,
  ...
  const [filteredData, setFilteredData] = useState<{ rows: any[]; total: number; states: StatsData } | null>(null);
  ...
  const handleOpenCustomize = () => {
    if (isFiltered) {
      ...
      ShowToast({color: 'warning', title: 'Not Filtered', description: 'Please apply filter first'})
    }
  }
  ...
  return (
    <div className="p-4 space-y-4 h-full">
      ...
      <ReportTable
        ...
        classNames={{
          tableWrapper: isHiddenState ? 'max-h-[calc(100vh-30rem)]' : 'max-h-[calc(100vh-10rem)]',
        }}
