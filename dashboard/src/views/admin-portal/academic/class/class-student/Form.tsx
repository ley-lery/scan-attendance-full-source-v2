...
const Form = ({ isOpen = false, onClose, loadList, isEdit, row }: FormProps) => {
  ...
  const { mutate: createClassStudent, loading: creating } = useMutation({
    onSuccess(res) {
      ...
      resetForm();
      loadList?.();
      ShowToast({title: t("success"), color: "success", description: res?.message[0]?.message });
    },
    onError(err) {
      console.log(err, "err");
      ShowToast({title: t("error"), color: "error", description: err?.response?.data?.message });
    },
  });
  ...
  const { data: formLoad } = useFetch<{ rows: any[]; total_count: number }>("/studentclass/listload");
  ...
  const validate = () => {
    ...
    if (!formData.classId) {
      errors.classId = t("validation.required");
    }

    if (!students.size) {
      errors.studentIds = t("validation.required");
    }
    return errors;
  };
  ...
  const onSubmit = async (): Promise<boolean> => {
    ...
    const payloadCreate = {
      ...
      studentIds: Array.from(students).map((id) => Number(id)),
      status: formData.status,
    };

    const payloadUpdate = {
      classId: Number(formData.classId),
      studentId: Number(Array.from(students)[0]), 
      status: formData.status,
    };
    ...
    >
      <div className="grid grid-cols-2 gap-4">
        ...
        <MultiSelect
          ...
          options={formLoad?.data?.students}
        ...
        <div className="col-span-2">
          <p className="text-small text-default-500">Selected: {Array.from(students).length}</p>
        </div>
        ...
        >
          {["Active", "Complete", "Inactive"].map((status) => (
            <div key={status} className="flex items-center gap-2">
              <Radio value={status} color={status === "Active" ? "primary" : status === "Complete" ? "success" : "danger"}>
                {status}
              </Radio>
            </div>
          ))}
