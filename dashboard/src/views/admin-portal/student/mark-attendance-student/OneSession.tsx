...
import { Chip, Spinner, type Selection } from "@heroui/react";
...
const Index = () => {
  ...
  const cols = useMemo(
    () => [
      ...
      { name: t("classStatus"), uid: "class_status", sortable: true },
  ...
  const validation = (formData: DataFilter, t: (key: string) => string) => {
  ...
  const applyFilterWithPagination = useCallback(
    async (page: number) => {
      const validationErrors = validation(filter, t);
      ...
      const payload: DataFilter = {
        ...
        classStudentStatus: filter.classStudentStatus,
  ...
  const colsKeys = useMemo(
    ...
    [customizeCols, customizeColSession, customizeColClassStudentStatus]
  );
  ...
  const filterSection =  <FilterData filter={filter} setFilter={setFilter} onApplyFilter={onFilter} onResetFilter={resetFilter} filterLoading={filterLoading} errors={errors} setErrors={setErrors} />
