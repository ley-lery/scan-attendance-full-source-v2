...
import { Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Chip, type Selection, type ChipProps, type SortDescriptor, Input, Pagination, Spinner, Tooltip, Select, SelectItem, ScrollShadow } from "@heroui/react";
...
import { IoCheckmarkCircleOutline, IoOptions } from "react-icons/io5";
...
const TABLE_CLASS_NAMES = {
  ...
  th: ["text-default-500 border-b border-divider sticky top-0 z-10 bg-zinc-100 dark:bg-zinc-900"],
  td: [
...
const DataTable = memo(({
  ...
  return (
    <div className="relative flex flex-col h-full">
      ...
      <AnimatePresence>
        {
          isOpenFilter && (
            ...
            >
              <div className="py-2">
                {/* bg-zinc-300/40 dark:bg-zinc-800 */}
      ...
      <div className={cn(
        "flex items-center justify-start flex-shrink-0 pt-4 gap-4",
        loading && "opacity-50",
      )}>        
      {
        totalPages > 1 && (
          <>
            <Pagination
              ...
              size="sm"
            />
            <span className="text-base text-default-400" >{page + ' of ' + totalPages} Pages</span>
          </>
        )
      }
      <Select
        ...
        onSelectionChange={(keys) => {
          ...
          if (selected) {
            handlePageSizeChange?.(Number(selected));
          }
        }}
        classNames={{ base: 'w-20', trigger: 'bg-zinc-200 dark:bg-zinc-800' }}
