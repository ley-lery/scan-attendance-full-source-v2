        ...
        <ModalContent>
          {() => (
            <>
              {
                loading && (
                  <div className={cn("flex items-center justify-center min-h-36 inset-0 absolute bg-white/20 dark:bg-black/20 z-50", loadingType === "blur" ? "backdrop-blur-sm" : "")}>
                  {hideIconLoading ? null : <Spinner variant="spinner" size="sm" label={isEdit ? t("updating") : t("saving")} />}
                </div>
                )
              }
