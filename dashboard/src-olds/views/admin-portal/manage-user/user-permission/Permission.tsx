...
const Permission = ({ isOpen = false, onClose, userId }: PermissionProps) => {
  ...
  const onApply = async () => {
    if (!userId) {
      ShowToast({ color: "danger", description: t("userIdRequired") });
      return;
    }
    ...
    try {
      ...
      await updatePermission('/userpermission', payload, "POST");
            ...
            <ModalBody>
              ...
              {userInfo && (
                <div className="mb-2 flex items-center gap-2">
                  <Avatar size="lg" name={userInfo.username} />
              ...
              <ScrollShadow className="has-scrollbar h-full max-h-96">
                <table className="w-full border-collapse">
                  <thead className="bg-white dark:bg-[#18181b]">
                 ...
                 {/* Student Leave Permissions: don't show action permission without actions request, approve, reject */}
                  <table className="w-full border-collapse">
                      ...
                      {Object.entries(groupedPermissions)
                        .filter(([resource]) => resource === "studentleave" || resource === "lecturerleave")
                        .map(([resource, perms]) => (
                          <tr key={resource}>
                            <td className="p-3 font-medium capitalize text-zinc-800 dark:text-zinc-100">
                              {t(resource)}
