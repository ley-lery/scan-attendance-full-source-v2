...
export const AuthModel = {
    ...
    async removeSession(token: string): Promise<any> {
        const sql = `Call sp_auth_user_remove_session(?, @p_messages_json)`;
        const params = [token];
        await db.query(sql, params);
        // Fetch the output parameter
        return Message.callProcedureWithMessages();
    ...
    async checkSession(token: string): Promise<any> {
        const sql = `Call sp_auth_user_check_session(?, @p_messages_json)`;
        const params = [token];
        await db.query(sql, params);
        // Fetch the output parameter consistently
        return Message.callProcedureWithMessages();
    },
