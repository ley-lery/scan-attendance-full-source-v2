...
import { StudentLeaveModel } from "./model";
...
import { StudentModel } from "../manage-student/model";
...
const model = StudentLeaveModel;


export const StudentLeaveController = {
    ...
    async getById(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            if (!rows?.length) {
                res.send({ message: "student leave not found" });
                return;
            }
    ...
    async approveLeave(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            if (messages && messages.length > 0 && messages[0].code === 0) {
                ...
                res.send({ message: messages[0].message || "Error approve student leave" });
            }
        } catch (e: any) {
    ...
    async rejectLeave(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            if (messages && messages.length > 0 && messages[0].code === 0) {
                ...
                res.send({ message: messages[0].message || "Error reject student leave" });
            }
    ...
    async batchLeave(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            if (messages && messages.length > 0 && messages[0].code === 0) {
                ...
                res.send({ message: messages[0].message || "Error batch student leave" });
            }
    ...
    async formLoad(req: FastifyRequest, res: FastifyReply): Promise<void> {
        try {
            ...
            sendSuccessResponse(res, true, "Form load", { faculties: faculties, fields: fields, students: students, classes: classes, users: users, status: status }, 200);
        } catch (e) {
