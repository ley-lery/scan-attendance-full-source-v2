...
export const MarkAttStudentController = {
    ...
    async filter(req: FastifyRequest, res: FastifyReply): Promise<void> {
        const data = req.body as ParameterData;
        try {
    ...
    async formLoad(req: FastifyRequest, res: FastifyReply): Promise<void> {
        try {
            ...
            sendSuccessResponse(res, true, "student sessions", { faculyties: faculyties, fields: fields, classes: classes, courses: courses, students: students, sessions: sessions, promotionNo: promotionNos, termNo: termNos, studentStatus: studentStatus, programType: programTypes, gender: genders, classStudentStatus: classStudentStatus, transferred: transferred }, 200);
        } catch (e) {
            handleError(res, e as Error, "Error fetching student sessions", 500);
        }
    },
