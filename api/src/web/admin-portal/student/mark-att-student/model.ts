...
export const AdminMarkAttStudentModel = {
    ...
    async getAll(page: number, limit: number): Promise<any> {
        const [result] = await db.query(`Call sp_admin_student_attendance_list_by_filters(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [null, null, null, null, null, null, null, null, null, null, null, 0, null,'s1', page, limit]);
        return result;
    },
    ...
    async filter(data: ParameterData): Promise<any> {
        const { faculty, field, classId, course, student, promotionNo, termNo, programType, gender, studentStatus, classStudentStatus, transferred, searchKeyword, sessionNo, page, limit } = data;
        const [result] = await db.query(
            `Call sp_admin_student_attendance_list_by_filters(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [faculty, field, classId, course, student, promotionNo, termNo, programType, gender, studentStatus, classStudentStatus, transferred, searchKeyword, sessionNo, page, limit]
        );
        return result;
    },
