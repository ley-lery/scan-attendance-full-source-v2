...
const initialFormData: UserPermission = {
  id: null,
  user: null,
  permission: null,
};
...
const Form = ({ isOpen = false, onClose, loadList, isEdit, row }: FormProps) => {
  ...
  const [formData, setFormData] = useState<UserPermission>(initialFormData);
  ...
  const { data: formLoad } = useFetch<{ users: any[], roles: any[] }>("/userpermission/formload");
    ...
    if (isOpen) {
      if (isEdit && row) {
        setFormData({
          ...
          permission: String(row.permission_id),
        });
      } else {
  ...
  const onSubmit = async (): Promise<boolean> => {
    const validationErrors = Validation.UserPermission(formData, t);
    ...
    try {
      ...
      if (isEdit) {
        await updateFaculty(`/userpermission/${id}`, formData, "PUT");
        ShowToast({ color: "success", description: t("updatedSuccess") });
      } else {
        await createFaculty(`/userpermission`, formData, "POST");
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <Autocomplete
            ...
            selectedKey={formData.permission}
            isInvalid={!!errors.permission}
            errorMessage={errors.permission}
            className="w-full"
            onSelectionChange={(key) =>
              handleInputChange({
                target: { name: "permission", value: key ?? "" },
              })
            }
            isRequired
          >
            {formLoad?.data?.permissions.map((item: any) => (
