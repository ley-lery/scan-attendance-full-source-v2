...
const Index = () => {
  ...
  const onDelete = async (id: number) => {
    try {
      ...
      await deleteRole(`/role/${id}`, id, "DELETE");
      await refetch();
      ShowToast({ color: "success", title: "Success", description: "Role deleted successfully" });

    } catch (error) {

      console.error(error);
      ShowToast({ color: "error", title: "Error", description: "Failed to delete role" });
  ...
  return (
    <div className="p-4">
      ...
      <DataTable
        ...
        loadData={refetch}
        selectRow={false}
        permissionCreate="create:role"
        permissionDelete="delete:role"
        permissionEdit="update:role"
        permissionView="view:role"
