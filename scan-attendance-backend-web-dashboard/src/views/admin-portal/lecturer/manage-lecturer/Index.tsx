...
const Index = () => {
  ...
  const { data, loading, refetch } = useFetch<{ rows: any[]; total_count: number }>(
    debouncedSearchKeyword.trim() === "" ? "/lecturer/list" : "/lecturer/search", 
    {
      params: {
  ...
  const { mutate: deleteLecturer } = useMutation();
  ...
  const onDelete = async (id: number) => {
    try {
      ...
      await deleteLecturer(`/lecturer/${id}`, id, "DELETE");
      await refetch();
      ShowToast({ color: "success", title: "Success", description: "Lecturer deleted successfully" });

    } catch (error) {

      console.error(error);
      ShowToast({ color: "error", title: "Error", description: "Failed to delete lecturer" });
