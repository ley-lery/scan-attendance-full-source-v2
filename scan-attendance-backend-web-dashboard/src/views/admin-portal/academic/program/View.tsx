...
const View = ({ isOpen = false, onClose, row }: ViewProps) => {
  ...
  const fields: { label: string; value: any; isStatus?: boolean; isDate?: boolean }[] = [
    ...
    { label: t("credits"), value: row?.credits },
    ...
    <ModalView isOpen={isOpen} onClose={onClose} title={t("viewDetail")} size="xl">
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {/* Labels */}
          <div className="space-y-2 text-sm font-medium text-zinc-700 dark:text-zinc-400">
            {fields.map((field) => (
          ...
          {/* Values */}
          <div className="space-y-2 text-sm font-normal text-zinc-500 dark:text-zinc-400">
            {fields.map((field) => {
              if (field.isStatus) {
                return (
