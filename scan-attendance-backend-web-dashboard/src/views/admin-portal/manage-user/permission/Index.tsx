...
const Index = () => {
  ...
  const { data, loading, refetch } = useFetch<{ rows: any[]; total_count: number }>(
    debouncedSearchKeyword.trim() === "" ? "/permission/list" : "/permission/search", 
    {
      params: {
  ...
  const cols = useMemo(
    () => [
      ...
      { name: t("permission"), uid: "name", sortable: true },
  ...
  const onDelete = async (id: number) => {
    try {
      ...
      await deletePermission(`/permission/${id}`, id, "DELETE");
      await refetch();
  ...
  return (
    <div className="p-4">
      ...
      <DataTable
        ...
        permissionEdit="update:permission"
        permissionView="view:permission"
