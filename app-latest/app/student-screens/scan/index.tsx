...
export default function Scan() {
    ...
    const handleQRData = useCallback(async (qrData: string) => {
        ...
        try {
            ...
            // Step 2: Fetch class list from API
            const classListResult = await apiService.current.fetchClassList(qrData);
            console.log("class List Result:", JSON.stringify(classListResult, null, 2));
            
            if (!classListResult.success) {
                handleApiError(classListResult.error!);
                return;
            }

            const classData: any = classListResult.data ;

            const payload: ScanData = {
                classId: classData.class_id,
                studentId: 4,
                dayOfWeek: moment().format("dddd"),
                targetTime: moment().format("HH:mm:ss")
            }

            // Step 6: Create attendance record
            const attendanceResult = await apiService.current.ProcessAttendance(payload);
            ...
            if (!attendanceResult.success) {
                ...
                if (attendanceResult.error) {
                    ...
                    // Handle case where there's a message but not marked as success
                    const apiError: ApiError = {
                        type: 'time_slot',
                        message: attendanceResult.data?.message || 'Attendance processing failed',
                        details: attendanceResult.data
                    };
    ...
    const getErrorMessage = useCallback(() => {
        ...
        switch (currentError.type) {
            ...
            case 'not_found':
                return t('error.classNotFound') || 'Class not found in this session';
            case 'already_scanned':
    ...
    return (
        <>
            ...
            <Alert
                ...
                onConfirm={() => router.replace("../(class)/location")}
            ...
            <Alert
                ...
                title={t("notFound")}
                message={t("desc.classNotFoundDesc")}
                onConfirm={() => {
                    notFoundModal.onClose();
                    handleRetry();
                }}
            ...
            <Alert
                ...
                message={t("desc.classScannedDesc")}
                onConfirm={() => {
                                ...
                                {isSuccess && attendanceData && (
                                    ...
                                    >
                                        ...
                                        <DetailRow 
                                            icon="person-outline" 
                                            label={t('class')} 
                                            value={attendanceData?.class_name_en || 'N/A'} 
                                        />
                                        <DetailRow 
                                            icon="book-outline" 
                                            label={t('subject')} 
