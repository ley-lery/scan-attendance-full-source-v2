...
import { Badge, BlurCard, Card, Icon, Loading, Notification, Text, Typography, useDisclosure, useHaptic, useToast } from '@/godui'
...
import { Alert, Clipboard, Image, Platform, SafeAreaView, ScrollView, Text as TextNative, TouchableOpacity, useColorScheme, View } from 'react-native'



const CardOthers = ({ icon, title, subtitle, navigate, isFocused }: { icon: string, title: string, subtitle: string, navigate: string, isFocused: boolean }) => (
...
const Home = () => {
  ...
  const SectionGradientCard = ({ children, style }: { children: React.ReactNode, style?: object }) => (
    <LinearGradient
    ...
    const fetchData = async () => {
      ...
      if (!permission) {
        show({
          ...
          message: t('locationPermissionMessageRejected'),
          type: 'error',
        })
      }else{
        show({
          title: t('locationPermission'),
          message: t('locationPermissionMessageGranted'),
          type: 'success',
        })
      }
      setIsLoading(false);
          ...
          >
            <View className="flex-row items-center mb-6">
              ...
              <View className="ml-4 flex-1">
                <Text className="text-zinc-600 dark:text-zinc-400 text-base font-normal">{t('welcomeBack')}!</Text>
            ...
            >
              <SectionGradientCard>
                <View className='py-4 px-6 gap-1'>
                  ...
                  <View>
                    <TextNative className="text-zinc-700 dark:text-white text-3xl font-bold">Quick Attendance Scan</TextNative>
                    <Typography variant="h2">UNISCAN</Typography>
                  </View>
                  <TextNative className="text-zinc-600 dark:text-zinc-300 text-sm">
                    Use your camera to scan the QR code from your instructor and mark your attendance instantly.
                  </TextNative>
                </View>
              </SectionGradientCard>
            </MotiView>
