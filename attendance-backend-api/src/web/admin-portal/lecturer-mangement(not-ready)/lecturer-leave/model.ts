...
export const LecturerModel = {
    ...
    async create(data: Lecturer): Promise<any> {
        const {lecturerCode, lecturerNameEn, lecturerNameKh, dob, gender, email, phone, password, status } = data;
        await db.query(`Call sp_lecturer_create(?, ?, ?, ?, ?, ?, ?, ?, ?, @p_messages_json)`, [lecturerCode, lecturerNameKh, lecturerNameEn, dob, gender, email, phone, password, status]);
        return Message.callProcedureWithMessages();
    },

    async update(id: number, data: Lecturer): Promise<any> {
         const {lecturerCode, lecturerNameEn, lecturerNameKh, dob, gender, email, phone, password, status } = data;
        await db.query(`Call sp_lecturer_update(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, @p_messages_json)`, [id, lecturerCode, lecturerNameKh, lecturerNameEn, dob, gender, email, phone, password, status]);
        return Message.callProcedureWithMessages();
    },
