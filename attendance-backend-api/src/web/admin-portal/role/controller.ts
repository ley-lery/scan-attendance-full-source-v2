...
export const RoleController = {
    async getAll(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [rows, [{ total } = { total: 0 }]] = await RoleModel.getAll(page, limit);
            if (!rows?.length) {
    ...
    async getById(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [rows] = await RoleModel.getById(id);
            if (!rows?.length) {
    ...
    async create(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            const [result] = await RoleModel.create(updatedData);
            ...
            if (messages && messages.length > 0 && messages[0].code === 0) {
                ...
                res.status(400).send({ message: messages[0].message || "Error creating role" });
            }
        } catch (e: any) {
    ...
    async update(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            const [result] = await RoleModel.update(id, updatedData);
            const messages = JSON.parse(result?.messages);
            if (messages && messages.length > 0 && messages[0].code === 0) {
                sendSuccessResponse(res, true, messages, null, 200);
            } else {
                res.status(400).send({ message: messages[0].message || "Error updating role" });
            }
        } catch (e: any) {
    ...
    async delete(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const [result] = await RoleModel.delete(id, updatedData);
    ...
    async search(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [ rows, [{total} = {total : 0} ]] = await RoleModel.search(keyword, page, limit);
            if (!rows?.length) {
