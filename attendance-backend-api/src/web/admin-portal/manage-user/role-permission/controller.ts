...
import { PermissionModel } from "../../manage-user/permission/model";

export const RolePermissionController = {
   
    async toggle(req: FastifyRequest, res: FastifyReply): Promise<void> {

        const data = req.body as { role: number; permissions: number[] };
        ...
        try {
            ...
            const result = await RolePermissionModel.toggle(data);
    ...
    async getByRoleId(req: FastifyRequest, res: FastifyReply): Promise<void> {
        const { id } = req.params as { id: number };
        try {
            const [result] = await RolePermissionModel.getByRoleId(id);
    ...
    async search(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [ rows, [{total} = {total : 0} ]] = await RolePermissionModel.search(keyword, page, limit);
            if (!rows?.length) {
    ...
    async getPermissionCurrentUser(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const [rows] = await RolePermissionModel.getPermissionCurrentUser(userPayload.user_id);
            if (!rows?.length) {
    ...
    async formLoad(req: FastifyRequest, res: FastifyReply): Promise<void> {
        try {
            ...
            sendSuccessResponse(res, true, "Form load", { permissions: permissions }, 200);
        } catch (e) {
