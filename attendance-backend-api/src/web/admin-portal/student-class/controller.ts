...
export const StudentClassController = {
    ...
    async getAll(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [rows, [{ total } = { total: 0 }]] = await StudentClassModel.getAll(page, limit);

            if (!rows?.length) {
                res.status(404).send({ message: "No students class found" });
                return;
            }
    ...
    async getById(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [rows] = await StudentClassModel.getById(id);

            if (!rows?.length) {
                res.status(404).send({ message: "student class not found" });
                return;
            }
    ...
    async create(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const [result] = await StudentClassModel.create(updatedData);
    ...
    async update(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const [result] = await StudentClassModel.update(id, updatedData);
            const messages = JSON.parse(result?.messages) ;
            if (messages && messages.length > 0 && messages[0].code === 0) {
                sendSuccessResponse(res, true, messages, null, 200);
            }else{
    ...
    async delete(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const [result] = await StudentClassModel.delete(id, updatedData);
    ...
    async search(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [ rows, [{total} = {total : 0} ]] = await StudentClassModel.search(keyword, page, limit);
            if (!rows?.length) {
