...
export const PermissionController = {
    async getAll(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [rows, [{ total } = { total: 0 }]] = await PermissionModel.getAll(page, limit);
            if (!rows?.length) {
                res.status(404).send({ message: "No permissions found" });
                return;
            }
    ...
    async getById(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [rows] = await PermissionModel.getById(id);
            if (!rows?.length) {
    ...
    async create(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            const [result] = await PermissionModel.create(updatedData);
            ...
            if (messages && messages.length > 0 && messages[0].code === 0) {
                ...
                res.status(400).send({ message: messages[0].message || "Error creating permission" });
            }
        } catch (e: any) {
    ...
    async generate(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            const [result] = await PermissionModel.generate(data, updatedData);
            const messages = JSON.parse(result?.messages);
            if (messages && messages.length > 0 && messages[0].code === 0) {
                sendSuccessResponse(res, true, messages, null, 200);
            } else {
                res.status(400).send({ message: messages[0].message || "Error generating permission" });
            }
        } catch (e: any) {
    ...
    async update(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            const [result] = await PermissionModel.update(id, updatedData);
            const messages = JSON.parse(result?.messages);
            if (messages && messages.length > 0 && messages[0].code === 0) {
                sendSuccessResponse(res, true, messages, null, 200);
            } else {
                res.status(400).send({ message: messages[0].message || "Error updating permission" });
            }
        } catch (e: any) {
    ...
    async delete(req: RequestWithUser, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const [result] = await PermissionModel.delete(id, updatedData);
    ...
    async search(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            const [ rows, [{total} = {total : 0} ]] = await PermissionModel.search(keyword, page, limit);
            if (!rows?.length) {
